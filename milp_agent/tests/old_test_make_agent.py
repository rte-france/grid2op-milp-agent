...
elif re.match("^l2rpn_neurips_2020_track2.*", env_name) is not None:
...
if re.match("^l2rpn_neurips_2020_track2_.*", env_name) is not None:
    ...
    env = orig_env["l2rpn_neurips_2020_track2_x2"]
else:
    env = orig_env
'''
env = orig_env
...
'''
    ...
    GLOBAL_SWITCH,
    max_overflow_percentage=np.ones(env.n_line) * 0.90,
    solver_name=milpagent.MIP_CBC,  # MIP_CBC LINEAR_GLOP MIP_SCIP MIP_GUROBI
...
for i in tqdm(range(nb_step), disable=False):
    ...
    if info["exception"] and not done:
        raise RuntimeError(info["exception"][0])
    # if act_this:
    #     import pdb
    #     pdb.set_trace()
    if done:
